...
The only step that can't be scripted: the CropFormer checkpoint is hosted on a **gated** HuggingFace dataset, so each person needs to request access and log in once via `hf auth login` (the notebook's Step 0 cell explains exactly how, and prints the error with next steps if you skip it).
